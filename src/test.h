...
void test_transform() {
    std::vector<std::vector<float>> m = {{0.1, 0.3}, {0.2, 0.7}, {0.8, 0.2}};
    std::vector<std::vector<uint8_t>> bit_map(m.size(), std::vector<uint8_t>(m[0].size()));
    std::transform(m.begin(), m.end(), bit_map.begin(), [](std::vector<float> p){std::vector<uint8_t> res(p.size());
...
void test_numpy() {
    ...
    cv::Point2f points[3] = {{4.1,5.3}, {5.5,0.2}, {1.2, 6.1}};
